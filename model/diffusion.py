...
class GradLogPEstimator(BaseModule):
    def __init__(self, dim_base, dim_cond, use_ref_t, dim_mults=(1, 2, 4)):
        ...
        self.pre_ppg = torch.nn.Conv1d(144, 80, 1)

    def forward(self, x, x_mask, ppg, ref, ref_mask, c, t):
        """
        x:   source mel
        ppg: source ppg
        ref: ref mel
        ...
        ppg = self.pre_ppg(ppg)
        x = torch.stack([ppg, x], 1)
...
class Diffusion(BaseModule):
    ...
    @torch.no_grad()
    def reverse_diffusion(self, z, mask, ppg, ref, ref_mask, c, n_timesteps, mode):
        h = 1.0 / n_timesteps
        xt = z * mask
        for i in range(n_timesteps):
            ...
            xt_ref = [self.compute_diffused_mean(ref, ref_mask, torch.zeros_like(ref), t)]
#            for j in range(15):
#                xt_ref += [self.compute_diffused_mean(ref, ref_mask, mean_ref, (j+0.5)/15.0)]
            xt_ref = torch.stack(xt_ref, 1)
            if mode == 'pf':
                dxt = -0.5 * (xt + self.estimator(xt, mask, ppg, xt_ref, ref_mask, c, time)) * (beta_t * h)
            else:
                if mode == 'ml':
                ...
                dxt -= self.estimator(xt, mask, ppg, xt_ref, ref_mask, c, time) * (1.0 + kappa) * (beta_t * h)
                dxt += torch.randn_like(z, device=z.device) * sigma
            xt = (xt - dxt) * mask
        return xt

    @torch.no_grad()
    def forward(self, z, mask, ppg, ref, ref_mask, c, n_timesteps, mode):
        if mode not in ['pf', 'em', 'ml']:
            print('Inference mode must be one of [pf, em, ml]!')
            return z
        return self.reverse_diffusion(z, mask, ppg, ref, ref_mask, c, n_timesteps, mode)

    def loss_t(self, x0, mask, ppg, x_ref, c, t):
        # 在更改的模型中，由于我们没有引进average mel，我们的diffusion从均值为0的高斯噪声开始
        xt, z = self.forward_diffusion(x0, mask, torch.zeros_like(x0), t)
        xt_ref = [self.compute_diffused_mean(x_ref, mask, torch.zeros_like(x_ref), t, use_torch=True)]
#        for j in range(15):
#            xt_ref += [self.compute_diffused_mean(x_ref, mask, mean_ref, (j+0.5)/15.0)]
        xt_ref = torch.stack(xt_ref, 1)
        z_estimation = self.estimator(xt, mask, ppg, xt_ref, mask, c, t)
    ...
    def compute_loss(self, x0, mask, ppg, x_ref, c, offset=1e-5):
        b = x0.shape[0]
        t = torch.rand(b, dtype=x0.dtype, device=x0.device, requires_grad=False)
        t = torch.clamp(t, offset, 1.0 - offset)
        return self.loss_t(x0, mask, ppg, x_ref, c, t)
