...
from params import n_mels, train_frames, sampling_rate
...
class VCDecDataset(torch.utils.data.Dataset):
    def __init__(self, data_dir, val_file, exc_file):
        ...
        self.test_speakers = get_test_speakers()
        self.speakers = [spk for spk in os.listdir(self.wav_dir)
                         if spk not in self.test_speakers]
        self.speakers = [spk for spk in self.speakers
                         if len(os.listdir(os.path.join(self.wav_dir, spk))) >= 10]
        random.seed(random_seed)
    ...
    def get_wavs(self, audio_id, spk):
        wav_path = os.path.join(self.wav_dir, spk, audio_id + '.wav')
...
class VCDecBatchCollate(object):
    def __call__(self, batch):
        ...
        wavs1 = torch.zeros((B, train_frames * sampling_rate // 100), dtype=torch.float32)
        ...
        for i, item in enumerate(batch):
            ...
            wavs1[i, :mel_length * sampling_rate // 100] = wav[starts1[i]*sampling_rate//100:(starts1[i]+mel_length)*sampling_rate//100]
